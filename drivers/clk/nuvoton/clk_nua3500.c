...
#include "regs-nua3500-clock.h"
#include "clk-nua3500.h"

DECLARE_GLOBAL_DATA_PTR;

struct __nua3500_clk_priv {
	void *base;
	const struct __nua3500_ccf_ops *ops;
	struct __nua3500_clk_gate_data *gate_data;
	struct __nua3500_wrpll_data *pwd;
	struct __nua3500_clk_data *data;
};

/* struct __nua3500_ccf_ops - clock operations */
struct __nua3500_ccf_ops {
	int (*set_rate)(struct __nua3500_clk_priv *pc,unsigned long rate,
	                unsigned long parent_rate);
	unsigned long (*round_rate)(struct __nua3500_clk_priv *pc,unsigned long rate,
	                            unsigned long *parent_rate);
	unsigned long (*recalc_rate)(struct __nua3500_clk_priv *pc,
	                             unsigned long parent_rate);
...
unsigned long CLK_SetPLLFreq(struct __nua3500_clk_priv *pc,
                             unsigned long PllSrcClk, unsigned long u64PllFreq)
{
	struct __nua3500_clk_data *data = pc->data;
	struct __nua3500_wrpll_data *pwd = pc->pwd;
...
unsigned long CLK_GetPLLFreq(struct __nua3500_clk_priv *pc,
...
static unsigned long nua3500_pll_recalc_rate(struct __nua3500_clk_priv *pc,
        unsigned long parent_rate)
{
	unsigned long ll;
...
static int nua3500_pll_set_rate(struct __nua3500_clk_priv *pc,
                                unsigned long rate,
                                unsigned long parent_rate)
{
	struct __nua3500_wrpll_data *pwd = pc->pwd;
...
static int nua3500_pll_clock_enable(struct __nua3500_clk_priv *pc, bool enable)
{
	struct __nua3500_wrpll_data *pwd = pc->pwd;
	struct __nua3500_clk_data *data = pc->data;
	u32 val;
...
static const struct __nua3500_ccf_ops nua3500_pll_clk_ops = {
	.set_rate = nua3500_pll_set_rate,
	.recalc_rate = nua3500_pll_recalc_rate,
	.enable_clk = nua3500_pll_clock_enable,
};

static struct __nua3500_clk_priv __nua3500_init_plls[] = {
	[capll] = {
		.ops = &nua3500_pll_clk_ops,
		.gate_data = &__clk_id_gate_sets[capll],
		.pwd = &__nua3500_capll_data,
	},
	[syspll] = {
		.ops = &nua3500_pll_clk_ops,
		.gate_data = &__clk_id_gate_sets[syspll],
		.pwd = &__nua3500_syspll_data,
	},
	[ddrpll] = {
		.ops = &nua3500_pll_clk_ops,
		.gate_data = &__clk_id_gate_sets[ddrpll],
		.pwd = &__nua3500_ddrpll_data,
	},
	[apll] = {
		.ops = &nua3500_pll_clk_ops,
		.gate_data = &__clk_id_gate_sets[apll],
		.pwd = &__nua3500_apll_data,
	},
	[epll] = {
		.ops = &nua3500_pll_clk_ops,
		.gate_data = &__clk_id_gate_sets[epll],
		.pwd = &__nua3500_epll_data,
	},
	[vpll] = {
		.ops = &nua3500_pll_clk_ops,
		.gate_data = &__clk_id_gate_sets[vpll],
		.pwd = &__nua3500_vpll_data,
	},
};

static ulong nua3500_pll_parent_rate(struct __nua3500_clk_priv *pc)
{
	ulong parent_rate;
...
ulong get_rate_reg(struct clk *clk, void *base)
{
	...
	struct ___nua3500_clk_reg_info reg_info;
	ulong ll = 0;
...
static ulong nua3500_clk_get_rate(struct clk *clk)
{
	struct __nua3500_clk_priv *pc = dev_get_priv(clk->dev);
	ulong rate = 0;

	pr_debug("%s\n", __func__);
	if(clk->id <= vpll) {
		if (ARRAY_SIZE(__nua3500_init_plls) <= clk->id)
			return -ENXIO;
		pc = &__nua3500_init_plls[clk->id];
		if (!pc->data || !pc->ops->recalc_rate)
			return -ENXIO;
		rate = pc->ops->recalc_rate(pc, nua3500_pll_parent_rate(pc));
	} else {
...
static ulong nua3500_clk_set_rate(struct clk *clk, ulong rate)
{
	ulong rate_val = 0;
	struct __nua3500_clk_priv *pc = dev_get_priv(clk->dev);
	...
	if(clk->id <= vpll) {
		...
		rate_val = pc->ops->recalc_rate(pc, nua3500_pll_parent_rate(pc));
		if (rate_val)
			return rate_val;
	} else {
...
static int nua3500_clk_enable(struct clk *clk)
{
	struct __nua3500_clk_priv *pc = dev_get_priv(clk->dev);
	int ret = 0;
	u32 regVal, sel_val;
...
static int nua3500_clk_disable(struct clk *clk)
{
	struct __nua3500_clk_priv *pc = dev_get_priv(clk->dev);
	int ret = 0;
	u32 regVal, sel_val;
...
static int nua3500_clk_probe(struct udevice *dev)
{
	int i, err;
	struct __nua3500_clk_priv *pc;
	struct __nua3500_clk_data *data = dev_get_priv(dev);
	...
	for (i = 0; i < ARRAY_SIZE(__nua3500_init_plls); ++i) {
		pc = &__nua3500_init_plls[i];
		pc->data = data;
...
static struct clk_ops nua3500_clk_ops = {
	.set_rate = nua3500_clk_set_rate,
	.get_rate = nua3500_clk_get_rate,
	.enable = nua3500_clk_enable,
	.disable = nua3500_clk_disable,
};

static const struct udevice_id nua3500_clk_ids[] = {
	{ .compatible = "nuvoton,nua3500-clk" },
	{ },
};

U_BOOT_DRIVER(nua3500_clk) = {
	.name = "nua3500_clk",
	.id = UCLASS_CLK,
	.of_match = nua3500_clk_ids,
	.ops = &nua3500_clk_ops,
	.priv_auto_alloc_size = sizeof(struct __nua3500_clk_data),
	.probe = nua3500_clk_probe,
