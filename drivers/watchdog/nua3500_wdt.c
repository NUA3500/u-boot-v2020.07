...
struct nua3500_wdt_priv {
	fdt_addr_t base;		/* WDT Register address */
	struct regmap *sys;		/* SYS base */
};

static void nua3500_reg_lock(struct nua3500_wdt_priv *priv)
{
	u32 el = current_el();
...
static int nua3500_reg_unlock(struct nua3500_wdt_priv *priv)
{
...
static int nua3500_wdt_reset(struct udevice *dev)
{
	struct nua3500_wdt_priv *priv = dev_get_priv(dev);
...
static int nua3500_wdt_start(struct udevice *dev, u64 timeout_ms, ulong flags)
{
	struct nua3500_wdt_priv *priv = dev_get_priv(dev);
	u32 reg = WDTEN | RSTEN;
	int lock;
...
static int nua3500_wdt_stop(struct udevice *dev)
{
	struct nua3500_wdt_priv *priv = dev_get_priv(dev);
	int lock;
...
static int nua3500_wdt_probe(struct udevice *dev)
{
	struct nua3500_wdt_priv *priv = dev_get_priv(dev);
...
static const struct wdt_ops nua3500_wdt_ops = {
	.start = nua3500_wdt_start,
	.stop = nua3500_wdt_stop,
	.reset = nua3500_wdt_reset,
};

static const struct udevice_id nua3500_wdt_match[] = {
	{ .compatible = "nuvoton,nua3500-wdt" },
	{ }
};

U_BOOT_DRIVER(nua3500_wdt) = {
	.name = "nua3500-wdt",
	.id = UCLASS_WDT,
	.of_match = nua3500_wdt_match,
	.priv_auto_alloc_size = sizeof(struct nua3500_wdt_priv),
	.probe = nua3500_wdt_probe,
	.ops = &nua3500_wdt_ops,
};
